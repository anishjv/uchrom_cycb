...
def _extract_mitotic_data(intensity, semantic, derivative, dilation_size=21):
    ...
    """
    dilated_semantic = binary_dilation(
        semantic == 1, structure=np.ones(dilation_size)
    ).astype(int)
...
def gauss_changept(smooth_intensity, derivative, semantic):
    ...
    try:
        ...
        mitotic_indices, cycb_in_mitosis, deg_rate, max_cycb, success = (
            _extract_mitotic_data(smooth_intensity, semantic, derivative)
        )
        if not success:
        ...
        popt, _ = curve_fit(
            falling_half_gaussian, t, deg_rate_fall, p0=[A0, mu0, sigma0]
        )
        ...
        r_squared = 1 - np.sum(residuals**2) / np.sum(
            (deg_rate_fall - np.mean(deg_rate_fall)) ** 2
        )
        ...
        if changept < 0:
            print(
                "[gauss_changept] Negative changept detected, likely due to poor curve fitting"
            )
            return np.nan
...
def peaks_changept(intensity, derivative, semantic):
    ...
    try:
        ...
        if start_idx >= end_idx:
            print(
                "[peaks_changept] Invalid time window: max intensity at or after last mitotic point"
            )
            return np.nan
...
def visualize_changepts(intensities, semantics, derivatives, changepts, max_plots=12):
    ...
    for i, idx in enumerate(random_indices):
        ...
        mitotic_indices, cycb_in_mitosis, deg_rate, max_cycb, success = (
            _extract_mitotic_data(intensity, semantic, derivative)
        )
        ...
        if success:
            ...
            if not np.isnan(changept):
                ax1.axvline(
                    ...
                    linestyle="--",
                    linewidth=2,
                    label=f"Change point: {changept:.1f}",
                )
