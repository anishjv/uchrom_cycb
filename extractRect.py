...
def residual(angle,data):
    ...
    # Convert angle to scalar if it's a numpy array
...
########################################################################
def findRotMaxRect(data_in,flag_opt=False,flag_parallel = False, nbre_angle=10,flag_out=None,flag_enlarge_img=False,limit_image_size=300,initial_angle=None):
    ...
    initial_angle: if provided, use this as the starting angle for optimization instead of brute force search
    '''
    ...
    if flag_opt:
        if initial_angle is not None:
            ...
            popt = optimize.fmin(residual, coeff1, args=(data,), xtol=0.01, ftol=1.e-5, disp=False)
            angle_selected = float(popt[0])  # Convert to scalar
...
#######################################
    ...
    #read image
    #----------------
    a = cv2.cvtColor(cv2.imread(image_name),cv2.COLOR_BGR2GRAY)
    #change with 0 and 1
    ...
    ax.imshow(aa ,origin='lower',interpolation='nearest')
